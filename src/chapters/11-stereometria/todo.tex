% NOTE (issue #9): this file is no longer \input by chapters/11.tex.
% It is author scratch space, not finished chapter prose:
%   - the \todofoot links below (Delta magazine articles) are genuine
%     reading-list pointers and can still be mined for future sections;
%   - the "Regular convex polyhedra" / "Nonregular uniform..." block is a
%     garbled Markdown paste of the Wikipedia "Uniform polyhedron" article
%     (mangled links, broken footnote markers) -- not salvageable as prose,
%     kept here only so the source material is easy to find if needed;
%   - the \section{Objętość} block below mixes untranslated Hartshorne
%     excerpts (Lennes polyhedra, Cauchy's/Süss' theorems) with page
%     pointers like "211:", "213: tw. dehna intro?, pelne na 217",
%     "216: arccos 1/3" -- those page pointers are worth keeping for
%     whoever writes the missing section 11.3 (volume/dissection material,
%     a natural lead-in to niezmiennik-dehna.tex/11.4), but the raw English
%     theorem statements themselves are not book prose and should not be
%     copied in verbatim.
