...
\index{aksjomat!leżenia pomiędzy}%
...
Mamy podział na
\begin{itemize}
    \item trójkąty ostrokątne (z trzema kątami ostrymi),
    \item prostokątne (z jednym kątem prostym),
    \item rozwartokątne (z jednym kątem większym niż prosty),
\end{itemize}
ale też na:
\begin{itemize}
\item równoboczne (z trzema bokami, które są przystające),
\item równoramienne (z dwoma przystającymi bokami zwanymi ramionami, wtedy trzeci bok nazywa się podstawą) albo
\item różnoboczne, z angielskiego \emph{scalene}.
\end{itemize}
...
\begin{definition}[półprosta]
    Niech $A$, $B$ będą punktami.
    Zbiór złożony z punktów $A$, $B$ oraz punktów, które leżą po tej samej stronie punktu $A$ na prostej $AB$ co punkt $B$, nazywamy półprostą nie ma sensownego oznaczenia
\index{półprosta}%
    % \begin{equation}
