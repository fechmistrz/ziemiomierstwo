...
Wprowadzamy w tej sekcji terminy: dylatacja (zwana homotetią), jednokładność, podobieństwo.
Przyjęte przez nas definicje nie muszą pasować do każdego podręcznika geometrii: po angielsku często mówi się, że \emph{homotheties} (nasze jednokładności) oraz \emph{translations} (nasze translacje) tworzą razem grupę \emph{dilations} (dylatacji).
...
Nazwy ,,dylatacja'' używa Coxeter \cite[s. 84]{coxeter_1967}: dwie figury są homotetyczne, jeśli są podobne i podobnie położone, to znaczy można je przekształcić przez dylatację lub homotetię.
...
Dylatacja z punktem stałym to jednokładność.
Pisze o nich Joanna Jaszuńska w $\Delta_{10}^3$.
% https://www.deltami.edu.pl/2010/03/dobrze-sie-sklada/

\begin{definition}[jednokładność]
    Jednokładność o środku $O$ oraz skali $k \neq 0$ to przekształcenie, które punktowi $A$ przypisuje punkt $A'$ taki, że $OA' = k \cdot OA$.
\end{definition}

\begin{example}
    Jednokładność o skali $1$ to identyczność, zaś o skali $-1$ to półobrót. 
\end{example}

Uogólnieniem dylatacji jest podobieństwo:
...
Eves \cite[s. 105]{eves1_1972} nazywa te przekształcenia \emph{homothety, expansion, dilatation or stretch}.

\begin{proposition}
    Niech $\Gamma_1, \Gamma_2$ będą dwoma różnymi okręgami o środkach $C_1, C_2$ oraz nierównych promieniach $r_1, r_2$.
    Wtedy dwie jednokładności przekształcają pierwszy okrąg na drugi:
    \begin{itemize}
    \item pierwsza o środku $O_1$ i skali $r_2/r_1$,
    \item druga o środku $O_2$ i skali $-r_2/r_1$.
    \end{itemize}
...
\begin{proposition}
    Każde dwa trójkąty podobne przekształcają się na siebie za pomocą dokładnie jednego podobieństwa.
\end{proposition}
...
To jest ćwiczenie u Bogdańskiej, Neugebauera \cite[s. 217]{neugebauer_2018}; stwierdzenie bez dowodu u Jaszuńskiej w $\Delta_{10}^3$.
