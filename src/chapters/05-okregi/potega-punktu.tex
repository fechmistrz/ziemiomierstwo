...
\subsection{Pęki okręgów}
Pękiem (z angielskiego \emph{pencil}) będziemy nazywać dowolną rodziną geometrycznych obiektów (przede wszystkim prostych lub okręgów) o pewnej wspólnej cesze, którą można odtworzyć z każdej pary obiektów wspomnianej rodziny.

\begin{example}
	Do pęku prostych należą wszystkie proste, które przechodzą przez ustalony punkt.
\end{example}

\begin{definition}
	Dowolne dwa niewspółśrodkowe okręgi $K_1, K_2$ wyznaczają jedną maksymalną rodzinę okręgów współosiowych z okręgami $K_1, K_2$, zwaną pękiem okręgów.
\end{definition}

O pękach okręgów pisze Neugebauer, Bogdańska \cite[s. 80]{neugebauer_2018}.
Dla ustalonch punktów $P, Q$ wprowadzają tam rodzinę
\begin{equation}
	\mathcal H(PQ) := \{A(\lambda) : 0 \le \lambda \le + \infty, \lambda \neq 1 \},
\end{equation}
gdzie $A(\lambda)$ to okrąg Apoloniusza z definicji \ref{definicja_okregu_apoloniusza}.
Okręgi $A(\lambda)$ są parami rozłączne, wszystkie są prostopadłe do wszystkich okręgów przechodzących przez punkty $P$ i $Q$.
Uznają, że $A(\infty)$ to punkt $Q$, $A(0)$ to punkt $P$, zaś $A(1)$ to symetralna odcinka $PQ$.
Wtedy przez każdy punkt płaszczyzny przechodzi dokładnie jeden okrąg $A(\lambda)$.
...
\begin{definition}
	Punkty $P, Q$ nazywamy punktami Ponceleta, prostą $PQ$ linią środków, zaś symetralną odcinka $PQ$ -- osią pęku hiperbolicznego $\mathcal H(PQ)$.
\end{definition}

Nazwisko Ponceleta pojawia się tu nieprzypadkowo, ma związek z (małym) twierdzeniem Ponceleta.

(Nie jest trudno się domyślić, że istnieją też pęki paraboliczne i eliptyczne).
...
Okręgi koncentryczne mają oś potęgową w nieskończoności.
